...
public class AquaFish
{
   ...
   public void moveForward(){
    ...
    {
        int topEdgeOfFish = myPos.yCoord() - (halfHeight + 1);
        return topEdgeOfFish;
    }
    
    public int distanceToBottom()
    {
        int bottomEdgeOfFish = myPos.yCoord() + (halfHeight + 1);
        return bottomEdgeOfFish;
    }
    

    
    public void ascend(){
    	int riseAmt = generator.nextInt(myHeight);
    	if (riseAmt > distanceToSurface()){
    		riseAmt=distanceToSurface();
    	}
    	myPos.rise(riseAmt);
    }

    public void descend(){
    	int fallAmt = generator.nextInt(myHeight);
    	if (fallAmt > distanceToBottom()){
    		fallAmt = distanceToBottom();
    	}
    	myPos.sink(fallAmt);
    }
    /**
